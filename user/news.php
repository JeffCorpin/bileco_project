                ...
                <?php foreach ($newsList as $news): ?>
                    <div class="bg-white p-4 rounded-md shadow-md">
                        ...
                        <?php if (strlen($news['content']) > 200): ?>
                            <button class="text-blue-500 continue-reading" onclick="openModal('<?php echo addslashes($news['title']); ?>', `<?php echo addslashes($content); ?>`, '<?php echo !empty($news['image']) ? '../uploads/' . addslashes($news['image']) : ''; ?>')">Continue Reading</button>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>
...
<div id="newsModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto relative">
